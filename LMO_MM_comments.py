...
1. Would be good to define what the offset vector is: I still don't quite understand your coordinate system.

2. I think your monte carlo algorithm accepts a spin flip automatically if delta_h > 0. Please check - this could lead to some unexpected behaviour. Because of the way you've written it, the sign of the energy change isn't clear to me. 

3. In get_molefraction, this 'counter' seems a rather complex data structure for what it is doing. The overall function is also inefficient - there should be no need to go through a triple nested for loop to get the mole fractions on each sublattice every time. Instead, we should design the array structure so this can be counted in one line by using numpy array operations in a single line (so we the remove the triple for loop). This can be done by defining an extra index array to distinguish the two sublattices, which is how my code works.

4. Thermal fluctuations: sample_frequency/total_iterations = 10. This number is far too low to get meaningful results.

5. Main: when saving files, use the relative path where possible, not the path on your own machine. This improves the portability of the code.
"""
